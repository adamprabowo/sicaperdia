              ...
              <div class="card-body">
                <table id="pindahTable" class="table table-bordered table-striped table-sm">
                  ...
                  <?php if (!empty($list_barang)) {
                    $no=1; foreach($list_barang as $lb){ ?>
                    <tr>
                    <td width="5%"><?=$no; ?></td>
                    <td><?=$lb->kode_barang; ?></td>
                    <td><?=$lb->nama_barang; ?></td>
                    <td><a class="btn btn-block btn-primary btn-xs" href="<?=base_url().'/laporan/lihatKartuStok/'.$lb->kode_barang?>"><i class="fas fa-file">
                              </i> Lihat kartu Stok</a></td>
                    </tr>
                  <?php $no++; }} ?>
                  </tbody>
                </table>
